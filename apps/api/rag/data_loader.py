...
def fetch_kyobo_new_releases() -> list[dict]:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"[RAG-DataLoader] 신간 API에서 {len(raw_books)}권의 도서 데이터를 성공적으로 수신했습니다.")
            for item in raw_books:
...
def get_vector_store() -> Chroma:
    ...
    api_success = False
    try:
        print("[RAG] 교보문고 일간 베스트셀러 데이터를 API로부터 직접 로드합니다...")
        bestsellers = fetch_kyobo_bestsellers()
        ...
        if len(merged_books) > 0:
            books = merged_books
            api_success = True
            print(f"[RAG] API 로드 완료 (중복 제거 후 총 {len(books)}권: 베스트셀러 {len(bestsellers)}권, 신간도서 {len(new_releases)}권)")
    except Exception as api_err:
        print(f"[RAG] 경고: API 로드 중 오류가 발생하여 정적 백업 데이터로 대체합니다. 오류 원인: {api_err}")
